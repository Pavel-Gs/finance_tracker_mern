...
const convertCSVToJSON = (csvData) => {
  ...
  return rows.map((row) => {
    ...
    return {
      amountExpense: parseFloat(columns[1]),
      typeExpense: columns[3].trim(),
      categoryExpense: columns[5].trim(),
      commentsExpense: columns[7].trim(),
      locationExpense: columns[9].trim(),
      organizationName: columns[11].trim(),
      dateExpense: parseDate(columns[13].trim()), // Use parseDate here for proper formatting
    };
  });
};
